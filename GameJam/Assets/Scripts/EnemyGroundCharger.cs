...
public class EnemyGroundCharger : MonoBehaviour {
  ...
  [SerializeField]
  float wallCheckDistance = 0.30f;  // forward ray
  [SerializeField]
  float ledgeCheckDistance = 0.60f;  // down ray
  [SerializeField]
  float skin = 0.04f;  // ray start offset outside collider
  [SerializeField]
  float groundedRadius = 0.08f;  // small circle at feet

  [Header("Turn Smoothing")]
  [SerializeField]
  float turnCheckHold = 0.10f;  // how long we must see a wall/ledge
  [SerializeField]
  float flipCooldown = 0.28f;  // lockout to stop ping-pong
  ...
  void Update() {
    ...
    // Decide to charge
    if (player &&
        Vector2.Distance(transform.position, player.position) <= aggroRange) {
  ...
  void Patrol() {
    ...
    // Don�t attempt fancy turning logic if we�re not grounded (falling /
    // stepping down)
    ...
    if (HasValidGroundMask()) {
      ...
      if (!warnedNoGroundMask) {
        Debug.LogWarning(
            $"[{name}] EnemyGroundCharger: groundMask is not set. " +
                "Ledge/Wall checks disabled. Assign your Ground layer.",
            this);
  ...
  Vector2 FeetPos() => new Vector2(BoundsCenter().x,
                                   MinY() + groundedRadius * 0.25f);

  bool IsGrounded() {
    if (!HasValidGroundMask())
      return true;  // don�t block movement if mask not set
    return Physics2D.OverlapCircle(FeetPos(), groundedRadius, groundMask) !=
           null;
  }

  // Cast your collider shape slightly forward; if we hit a surface whose normal
  // faces us, it's a wall.
  bool IsWallAhead() {
    if (!HasValidGroundMask())
      return false;
    ...
    float sweep =
        Mathf.Max(wallCheckDistance,
                  Mathf.Abs(rb.linearVelocity.x) * Time.deltaTime + 0.02f);

    ContactFilter2D filter =
        new ContactFilter2D { useTriggers = false, useLayerMask = true,
                              layerMask = groundMask };
    ...
    for (int i = 0; i < count; i++) {
      // If the surface normal faces against our travel dir, it's a blocking
      // wall
  ...
  bool IsLedgeAhead() {
    Vector2 front =
        new Vector2(BoundsCenter().x + dir * (ExtX() + skin), MinY() + skin);
    RaycastHit2D hit =
        Physics2D.Raycast(front, Vector2.down, ledgeCheckDistance, groundMask);
  ...
  void OnDrawGizmosSelected() {
    ...
    Vector2 wallOrigin =
        ...
        new Vector2(d * (col.bounds.extents.x + skin * 0.5f), 0f);
    Gizmos.DrawLine(wallOrigin,
                    wallOrigin + new Vector2(d, 0f) * wallCheckDistance);

    Gizmos.color = Color.yellow;  // ledge
    Vector2 ledgeOrigin =
        new Vector2(col.bounds.center.x + d * (col.bounds.extents.x + skin),
                    col.bounds.min.y + skin);
    Gizmos.DrawLine(ledgeOrigin,
                    ledgeOrigin + Vector2.down * ledgeCheckDistance);
  }
#endif
}